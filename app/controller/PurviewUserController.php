...
/**
 * Class PurviewUserController
 *
 * @package Controller
 */
class PurviewUserController extends ActiveController
{
	...
	public function actionUpdate(Request $request){
		$model = PurviewUser::findOne($request->input->integer('id'));
		if (empty($model)) {
    ...
    public function actionDetail(Request $request){
        ...
        $model = PurviewUser::findOne($check);
        if(empty($model)){
    ...
    public function actionDelete(Request $request){
		...
		$model = PurviewUser::findOne($_key);
		if (empty($model)) {
    ...
    {
        $pWhere = array();
        $pWhere['userId']      = $request->input->get('userId', false);                      //用户组名
        $pWhere['groupId']     = $request->input->get('groupId', false);                     //组ID
	    ...
	    //列表输出
	    $model = PurviewUser::where($pWhere)->orderBy($order);
	    if($count != -100){
