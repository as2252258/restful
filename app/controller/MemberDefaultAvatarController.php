<?php

namespace app\controller;

use app\components\ActiveController;
use app\model\MemberDefaultAvatar;
use Code;
use Exception;
use yoc\core\Str;
use yoc\http\Request;
use yoc\http\Response;

/**
 * Class MemberDefaultAvatarController
 *
 * @package Controller
 */
class MemberDefaultAvatarController extends ActiveController
{
	
	/**
	 * @param Request  $request
	 * @param Response $response
	 *
	 * @throws Exception
	 */
	public function actionAdd(Request $request)
	{
		$model = new MemberDefaultAvatar();
		$model->setBatch([
			'avatar' => $request->input->string('avatar' , false , [0 , 100]) ,          //头像地址
		]);
		$results = $model->save();
		if (!$results) {
			throw new Exception($model->getLastError());
		}
		return Response::analysis(Code::SUCCESS , $results);
	}
	
	/**
	 * @param Request  $request
	 * @param Response $response
	 *
	 * @throws Exception
	 */
	public function actionUpdate(Request $request)
	{
		$model = MemberDefaultAvatar::findOne($request->input->integer('id'));
		if (empty($model)) {
			throw new \Exception('指定数据不存在');
		}
		$model->setBatch([
			'avatar' => $request->input->string('avatar' , false , [0 , 100]) ,          //头像地址
		]);
		$results = $model->save();
		if (!$results) {
			throw new \Exception($model->getLastError());
		}
		return Response::analysis(Code::SUCCESS , $results);
	}
	
	/**
	 * @param Request  $request
	 * @param Response $response
	 *
	 * @throws Exception
	 */
	public function actionDetail(Request $request)
	{
		$check = $request->input->integer('id');
		if (empty($check)) {
			throw new Exception('param id can not empty');
		}
		$model = MemberDefaultAvatar::findOne($check);
		if (empty($model)) {
			throw new Exception('Data Not Exists');
		}
		return Response::analysis(STATUS_SUCCESS , $model);
	}
	
	/**
	 * @param Request  $request
	 * @param Response $response
	 *
	 * @throws Exception
	 */
	public function actionDelete(Request $request)
	{
		$_key = $request->input->integer('id' , true);
		$pass = $request->input->password('password' , true);
		if (empty($this->user) || strcmp(Str::encrypt($pass) , $this->user->password)) {
			throw new \Exception('密码错误');
		}
		$model = MemberDefaultAvatar::findOne($_key);
		if (empty($model)) {
			throw new \Exception('数据不存在');
		}
		if (!$model->delete()) {
			return Response::analysis(Code::ERROR , $model->getLastError());
		}
		return Response::analysis(Code::SUCCESS , 'delete success');
	}
	
	/**
	 * @param Request  $request
	 * @param Response $response
	 *
	 * @throws Exception
	 */
	public function actionList(Request $request)
	{
		$pWhere = [];
		$pWhere['avatar'] = $request->input->get('avatar' , false);                      //头像地址
		
		//分页处理
		$count = $request->input->get('count' , false , -1);
		$order = $request->input->get('order' , false , 'id');
		if (!empty($order)) {
			$order .= $request->input->get('isDesc') ? ' asc' : ' desc';
		} else {
			$order = 'id desc';
		}
		
		//列表输出
		$model = MemberDefaultAvatar::where($pWhere)->orderBy($order);
		if ($count != -100) {
			$model->limit($request->input->page , $request->input->size);
		}
		if ((int) $count === 1) {
			$count = $model->count();
		}
		return Response::analysis(Code::SUCCESS , $model->all() , $count);
	}
	
	/**
	 * @param \yoc\http\Request $request
	 *
	 * @return array
	 * 批量保存
	 */
	public function actionBatch(Request $request)
	{
		$list = $request->input->array('avatar' , true);
		if (empty($list) || !is_array($list)) {
			return Response::analysis(Code::DATA_EMPTY);
		}
		$default = MemberDefaultAvatar::where('id <> 0')->all();
		$default = array_column($default->toArray() , 'avatar');
		foreach ($list as $key => $val) {
			if (in_array($val , $default)) continue;
			$memberAvatar = new MemberDefaultAvatar();
			$memberAvatar->avatar = $val;
			$memberAvatar->save();
		}
		return Response::analysis(MemberDefaultAvatar::all());
	}
}