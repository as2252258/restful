...
/**
 * Class JournalCommentIdentifyController
 *
 * @package Controller
 */
class JournalCommentIdentifyController extends ActiveController
{
	...
	public function actionAdd(Request $request){
		$model = new DiaryCommentIdentify();
		$model->setBatch([
	...
	public function actionUpdate(Request $request){
		$model = DiaryCommentIdentify::findOne($request->input->integer('id'));
		if (empty($model)) {
    ...
    public function actionDetail(Request $request){
        ...
        $model = DiaryCommentIdentify::findOne($check);
        if(empty($model)){
    ...
    public function actionDelete(Request $request){
		...
		$model = DiaryCommentIdentify::findOne($_key);
		if (empty($model)) {
    ...
    public function actionList(Request $request)
    {
	    ...
	    //列表输出
	    $model = DiaryCommentIdentify::where($pWhere)->orderBy($order);
	    if($count != -100){
