...
/**
 * Class GroupIdentifyController
 *
 * @package Controller
 */
class GroupIdentifyController extends ActiveController
{
	...
	public function actionAdd(Request $request){
		$model = new GroupIdentify();
		$model->setBatch([
            'alias'   => $request->input->string('alias', false, [0,200]),           //权限别名
            'route'   => $request->input->string('route', false, [0,50]),            //
		]);
		$results = $model->save();
		if (!$results) {
	...
	public function actionUpdate(Request $request){
		$model = GroupIdentify::findOne($request->input->integer('id'));
		if (empty($model)) {
    ...
    public function actionDetail(Request $request){
        ...
        $model = GroupIdentify::findOne($check);
        if(empty($model)){
    ...
    public function actionDelete(Request $request){
		...
		$model = GroupIdentify::findOne($_key);
		if (empty($model)) {
    ...
    {
        $pWhere = array();
        $pWhere['alias']  = $request->input->get('alias', false);                       //权限别名
        $pWhere['route']  = $request->input->get('route', false);                       //
	    ...
	    //列表输出
	    $model = GroupIdentify::where($pWhere)->orderBy($order);
	    if($count != -100){
